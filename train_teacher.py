...
def main():
    ...
    logs_train_dir = './model/save_teacher/model.ckpt'
    path = 'SF.txt'
    num_per = 10
    ...
    code = tf.sign(feature_t3)

    hash_loss, _, _ = Hash_loss(feature_t3, label, batch_size, omega_size)
    q_loss = tf.reduce_mean(tf.pow(tf.subtract(feature_t3, code), 2.0))
